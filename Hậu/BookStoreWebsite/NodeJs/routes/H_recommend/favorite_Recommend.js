...
//Đếm thể loại trong từng OrderDetail của User
async function CreateDataCategoryCount(data, orderDetail, user, category) {
    let isExist1 = (data2, orderDetailCheck, userCheck, bookCategory) => {
        for (var key in data2) {
            if (data2[key].userID == userCheck.userID) { //check UserID
                if (data2[key].categoryID == bookCategory) {
                    data2[key].count += orderDetailCheck.count;
                    return data2;
                }
            }
        ...
        var dataReturn = {}
        dataReturn = { "userID": userCheck.userID, "categoryID": bookCategory, "count": orderDetailCheck.count }
...
router.get('/Data', function(req, res) {
        async function run() {
            ...
            // id user
            var author_recommend = await recommendation_eng(DataAuthor, isExistAuthor, "5eb5804d583fc710fc965457", pearson_correlation_author);
            var category_recommend = await recommendation_eng(DataCategory, isExistCategory, "5eb5804d583fc710fc965457", pearson_correlation_category);
            res.json({ author: author_recommend, category: category_recommend })
        }
        run();
    })
    //#endregion
...
//Kiểm tra tồn tại by book (author)
async function isExistAuthor(data, element) {
...
//Kiểm tra tồn tại by book (category)
async function isExistCategory(data, element) {
    let isExist1 = (data2, x) => {
        for (var key in data2) {
            if (data2[key].categoryID == x.categoryID) {
                return true;
            }
        } {
            return false;
...
//#region recommend 2 user (author)
async function pearson_correlation_author(dataset, p1, p2) {
...
//#region recommend 2 user (category)
async function pearson_correlation_category(dataset, p1, p2) {
    ...
    for (var key1 in Person1) {
        for (var key2 in Person2) {
            if (Person1[key1].categoryID == Person2[key2].categoryID) {
                existp1p2[Person1[key1].categoryID] = 1
                break;
            }
        }
    }
    ...
    for (var key in existp1p2) {
        for (var key1 in Person1) {
            if (key == Person1[key1].categoryID) {
                p1_cur = Person1[key1].count;
                p1_sum += Person1[key1].count;
                p1_sq_sum += Math.pow(Person1[key1].count, 2);
                break;
        ...
        for (var key2 in Person2) {
            if (key == Person2[key2].categoryID) {
                p2_cur = Person2[key2].count;
                p2_sum += Person2[key2].count;
                p2_sq_sum += Math.pow(Person2[key2].count, 2);
                break;
...
async function recommendation_eng(dataset, isExist, person, pearson_correlation) {
    ...
    for (var other in datafilter) {
        ...
        for (var data1 in dataPerson1) {
            if (!(await isExist(dataPerson2, dataPerson1[data1]))) {
                totals.setDefault(Object.values(dataPerson1[data1])[1], dataPerson1[data1].count * similar);
                simsum.setDefault(Object.values(dataPerson1[data1])[1], similar);
            }
        }
    }
